...
void showLidarTopview()
{
    ...
    for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it)
    {
        ...
        // Remove lidar points on the roads surface
        float zw = (*it).z; // height above road surface, world position in m with y facing left from sensor
        if(zw > -1.40){
            float val = it->x; // distance in driving direction
            float maxVal = worldSize.height;
            // Color Lidar points such that X=0.0m is red while X=20.0m is shown as green
